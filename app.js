...
async function loadData(direction) {
    ...
    try {
        ...
        // Оборачиваем её в прокси на Vercel (вызов Serverless Function)
        const proxyUrl = `/api/proxy?url=${encodeURIComponent(yandexUrl)}`;

        const [busRes, trainRes] = await Promise.all([
            fetch(busFile),
            fetch(proxyUrl) // Запрашиваем данные через прокси Vercel
        ]);
        const buses = await busRes.json();
        const trainData = await trainRes.json();
        
        // 3. Исключение дублирующихся рейсов электричек
        const uniqueTrains = [];
        const seen = new Set();
        for (const t of trainData.segments) {
            const tDep = t.departure.split('T')[1].substring(0, 5);
            const tArr = t.arrival.split('T')[1].substring(0, 5);
            const key = `${tDep}_${tArr}`;
            if (!seen.has(key)) {
                seen.add(key);
                uniqueTrains.push(t);
            }
        ...
        renderResults(direction, buses, uniqueTrains);
    } catch (e) {
...
function renderResults(dir, buses, trains) {
    ...
    if (dir === 'forward') {
        trains.forEach(t => {
            const tDep = t.departure.split('T')[1].substring(0, 5);
            const tArr = t.arrival.split('T')[1].substring(0, 5);
            const suitableBus = buses.find(b => b.departure_mozhaysk >= tArr);
            if (!suitableBus) return; // 1. Исключаем отрицательное время
        ...
        buses.forEach(b => {
            const suitableTrain = trains.find(t => t.departure.split('T')[1].substring(0, 5) >= b.arrival_mozhaysk);
            if (!suitableTrain) return; // 1. Исключаем отрицательное время
    ...
    cardsData.forEach((c, idx) => {
        ...
        // 4. Форматируем время ожидания
        const formattedWait = formatWaitTime(c.wait);
        ...
        card.innerHTML = c.type === 'forward' ? `
            <div class="timeline">
                ...
                <div class="step">🚶 Пересадка: ${formattedWait}</div>
                <div class="step">🚌 <span class="time">${c.bDep}</span> — <span class="time">${c.bArr}</span> (МАРШРУТКА - РАДУГА)</div>
            </div>
            ${c.wait < 15 ? `<div class="badge badge-tight">⚠️ ${formattedWait} на пересадку</div>` : ''}
    ...
    html2canvas(document.getElementById('capture-area')).then(canvas => {
        canvas.toBlob(blob => {
            ...
            if (navigator.canShare && navigator.canShare(shareData)) {
                navigator.share(shareData).catch(() => fallbackDownload());
            } else {
